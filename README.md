...
当前公开版本：`0.3.0`。已验证 DSH `0.1.1-rc.2`、Node.js `>=22.19.0`、pnpm `11.7.0`。
...
从 GitHub Releases 下载本仓库发布的 `dsh-workbench-0.3.0.tgz` 和对应 `.sha256`，校验后安装本地文件：

```bash
shasum -a 256 -c dsh-workbench-0.3.0.tgz.sha256
...
- 点击项目行的分支图标进入“项目会话图谱”；拖动会话卡片可整理位置，画布支持平移、缩放和自动适配。布局只保存在当前浏览器，不包含会话正文或工具数据。
