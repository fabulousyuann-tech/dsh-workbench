...
describe("UI contract", () => {
  ...
  it("普通会话与工作台会话互斥，客户和项目内可直接进入与删除会话", async () => {
    ...
    expect(panel).not.toContain('className="customerExpand"');
    expect(panel).not.toContain('className="projectExpand"');
    expect(panel).toContain('className="customerToggle"');
    expect(panel).toContain('aria-expanded={shownExpanded}');
    expect(panel).toContain('setExpandedOverride(!shownExpanded)');
    expect(panel).toContain('className="rowAction customerNewSessionAction"');
    expect(panel).toContain('t("sessions.customer.newChat")');
    expect(panel).toContain('className="customerMoreMenuRoot"');
    expect(panel).toContain('t("customer.more")');
    expect(panel).toContain("await startFolderSession(folderPath)");
    ...
    expect(panel).toContain('className="rowAction projectNewSessionAction"');
    expect(panel).toContain('t("sessions.project.newChat")');
    expect(panel).toContain('aria-expanded={expanded}');
    expect(panel).toContain('onClick={onToggle}');
    expect(panel).toContain('<Menu');
    expect(panel).toContain('className="projectMoreMenuRoot"');
    expect(panel).toContain("portal");
    expect(panel).toContain('{ type: "separator", id: "destructive" }');
    expect(panel).toContain("createFolderSession(project.folderPath)");
    expect(panel).not.toContain('className="rowAction rowDeleteAction"');
    ...
    expect(root).toContain("startFolderSession={startFolderSession}");
    ...
    expect(panel).toContain('const [expandedOverride, setExpandedOverride] = useState<boolean>()');
    expect(panel).toContain('const expanded = expandedOverride ?? selected');
    expect(panel).toContain('const shownExpanded = expandedOverride ?? (forceExpanded || activeWithin)');
