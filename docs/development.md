...
npx --yes @deepseek-ai/dsh@0.1.2-rc.1 web --port 3456 --no-open
```
...
当前兼容基线是 `0.1.1-rc.2`、`0.1.2-alpha.1`、`0.1.2-alpha.2` 与 `0.1.2-rc.1`；开发依赖固定为 `0.1.2-rc.1`，以当前 RC 的拆分 Controller/Remote 契约完成静态检查。旧版兼容通过能力探测和专门测试维持。
Workbench 不再注入已移除的 `dsh-client-runtime`。DSH 0.1.2 由官方 `ui-session`、`ui-renderer` 与 `ui-workspace` 提供 Session/Workspace selector hooks 和 Slot 服务；模型与历史读取优先使用 `remote.session`，0.1.1-rc.2 回退到 `connection.api`。
完整矩阵与 API 结论见
[ADR 0001](./adr/0001-dsh-integration-boundaries.md)。

## Sidebar 扩展插槽

Workbench 替代官方 Sidebar 时继续渲染 DSH 的公开子插槽，并额外提供一个可选列表插槽。DSH Desktop 扩展窗口或增强模式强制启用官方 Sidebar 时，Workbench 改为注册 `sidebar.workspaces` 内容区，不声明也不渲染官方外壳拥有的设置与页脚插槽：

| 插槽 | owner props | 布局 |
| --- | --- | --- |
| `sidebar.settings.trailing` | `{ wide: boolean }` | 展开时位于“设置”右侧；折叠时位于“设置”上方 |

`sidebar.settings.trailing` 仅在 Workbench 拥有完整 Sidebar 的普通 Web/兼容模式中存在；Desktop 扩展窗口与增强模式使用官方 Sidebar 原生页脚。
